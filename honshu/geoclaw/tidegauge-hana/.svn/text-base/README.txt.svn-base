...
<h2>
Tide Gauge Hana at 43.28N 145.57E
</h2>

Some sample results are shown at
[http://www.clawpack.org/links/honshu2011].
The results posted 13 March 2011 were computed with this code as it appears
in Revision 18 of the svn repository:

[http://kingkong.amath.washington.edu/svn/tsunami-benchmarks/honshu/geoclaw/tidegauge-hana]


Using the UCSB source parameters.  The USGS parameters seem to give poorer
agreement.  This can be switched in [code: setrun.py].
You also need to switch the dtopo file created in [code:maketopo.py], which is
used when "make topo" is run.
...
[link: honshu-usgs.txt] and [link: honshu-ucsb.txt].
These were downloaded from the following sites on 11 March 2011.
